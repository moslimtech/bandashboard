// // merged script.js — updated to handle server-side change (image names saved, links not written)
...
// // keep a short-lived cache of just-uploaded files so we can show them immediately
// // structure: { filename: { url, name } }
// const recentUploads = {};
...
//         opt.value = p.id;
//         const dur = p.duration || (p.raw && (p.raw['مدة الباقة باليوم'] || p.raw['مدة']) ? (p.raw['مدة الباقة باليوم'] || p.raw['مدة']) : '');
//         opt.textContent = `${p.name} (${dur} يوم)`;
...
//   const form = new FormData();
//   form.append('action', 'uploadFile'); // code.gs accepts uploadFile or uploadMedia
...
//   // cache the upload by filename so UI can use it immediately after addAd
...
//     // store recent uploads in cache for immediate UI rendering by filename
...
//       // immediately refresh UI: prefer reloading list, but also try to update the single card
...
//       // server often doesn't persist links in sheet (server change) — show the new ad immediately using local upload URLs
//       const returned = (data && data.data) ? data.data : data;
//       const newAdTemp = {
...
//         images: imageUrls.map(i => ({ name: i.name, url: i.url })), // immediate visible images
//         videoUrl: videoUrl || ''
//       };
//       showSuccess('تم حفظ الإعلان');
//       // render it at top of list for immediate feedback
...
//     // images: try url first, otherwise show a filename placeholder
...
//         // im might be { name, url } or { name, url:'' } or a plain string (legacy)
//         let url = '', name = '';
//         if (im && typeof im === 'object') { url = im.url || ''; name = im.name || ''; }
//         else if (typeof im === 'string') { name = im; url = ''; }

//         // If no url but we have a recent upload for that filename, use it
...
//         } else if (name) {
//           // show filename placeholder with icon
//           const wrap = document.createElement('div'); wrap.className = 'img-placeholder-file';
...
// // Prepend a new ad object to the list (used to show immediate feedback after add)
// function prependAdToList(ad) {
//   const container = document.getElementById('adsListContainer');
//   if (!container) return;
//   // render a single card similar to renderAdsList but insert at top
//   const card = document.createElement('div'); card.className = 'ad-card';
//   const h = document.createElement('h4'); h.textContent = ad.title || '(بدون عنوان)';
//   const meta = document.createElement('div'); meta.className = 'meta'; meta.textContent = `${ad.startDate || ''} — ${ad.endDate || ''} · الحالة: ${ad.status || ''}`;
//   const p = document.createElement('p'); p.textContent = ad.description || '';
...
//     });
//   } else {
//     methodsContainer.textContent = 'لا توجد طرق دفع معرفة. تواصل مع الإدارة للترتيب.';
//   }
...
//         msg.textContent = 'تم رفع الإيصال لكن فشل ربطه بطلب الدفع في النظام. سيتم إعلام الإدارة يدوياً.';
//         setTimeout(()=> modal.remove(), 2200);
//         return;
//       }

//       msg.textContent = 'تم إرسال الإيصال بنجاح. سيتم التحقق من الإدارة وتفعيل الباقة بعد التأكيد.';
...
// /* small helper to escape HTML for injected strings */
...
//       if (!status) return;
//       await updatePlaceStatus(status, btn); // immediate
//     });
//   });
// }
...
//   // ضبط حالة زر التفعيل عند التحميل الأولي
...
//   // زر تفعيل الاشتراك من نموذج المكان
//   const activatePackageBtn = document.getElementById('activatePackageBtn');
//   if (activatePackageBtn) {
//     activatePackageBtn.addEventListener('click', activatePackageFromForm);
//   }

//   // تحديث حالة زر التفعيل عند تغيير اختيار الباقة
//   const pkgSelect = document.querySelector('select[name="package"]');
//   if (pkgSelect) {
//     pkgSelect.addEventListener('change', updateActivateButtonState);
//   }
// }
...
//     // بعد ملء القوائم، حدّث حالة زر تفعيل الاشتراك
//     updateActivateButtonState();
//   } catch (err) {
//     console.error('loadLookupsAndPopulate error', err);
//   }
...
//   // عند تغيير حالة الدخول، حدّث حالة زر التفعيل
//   updateActivateButtonState();
// }
...
//     showError(err.message || 'فشل تغيير الباقة');
//   } finally {
//     // بعد محاولة التفعيل، حدّث حالة الزر
//     updateActivateButtonState();
//   }
// }
...
// /* ================= تفعيل الباقة من نموذج المكان + إدارة حالة الزر ================= */
...
//     const hasPackage = !!(select && select.value && String(select.value).trim() !== '');

//     btn.disabled = !(hasLogin && hasPackage);
//     btn.style.opacity = btn.disabled ? '0.6' : '1';

//     if (hint) {
//       if (!hasLogin) hint.textContent = 'سجّل الدخول أولاً لتفعيل الاشتراك';
//       else if (!hasPackage) hint.textContent = 'اختر باقة من القائمة ثم اضغط "تفعيل الاشتراك"';
...
//     // تعبئة الباقات: الاسم — المدة — السعر — عدد الإعلانات
//     const pkgSelect = document.querySelector('select[name="package"]');
//     if (pkgSelect) {
//       pkgSelect.innerHTML = '<option value="">اختر الباقة</option>';
//       (data.packages || []).forEach(p => {
...
// /* ========== تفعيل الباقة من النموذج + منع التجريبية أكثر من مرة ========== */
// async function activatePackageFromForm() {
//   try {
//     const packageSelect = document.querySelector('select[name="package"]');
//     const loggedPlace = getLoggedPlace();
...
//     const isTrialUsed = await checkIfTrialIsUsed(loggedPlace.id);
//     const selectedOpt = packageSelect.options[packageSelect.selectedIndex];
//     const selPrice = Number(selectedOpt?.dataset?.price || 0);
//     if (selPrice === 0 && isTrialUsed) {
//       showError('تم استخدام الباقة التجريبية مسبقاً. لا يمكن تفعيلها مرة أخرى.');
//       return;
//     }
...
//     const val = place.raw['حالة الباقة التجريبية'];
...
//       if (hint) hint.textContent = 'سجّل الدخول أولاً لتفعيل الاشتراك';
//       return;
//     }
//     if (!hasPackage) {
//       btn.disabled = true; btn.style.opacity = '0.6';
//       if (hint) hint.textContent = 'اختر باقة من القائمة ثم اضغط "تفعيل الاشتراك"';
//       return;
//     }
...
//         if (used) {
//           btn.disabled = true; btn.style.opacity = '0.6';
//           if (hint) hint.textContent = 'الباقة التجريبية تم تفعيلها مسبقًا.';
//         } else {
...
//       clearPackageCountdown();
//       // بعد انتهاء العد التنازلي، حدّث واجهة الحالة
...
//   }
//   tick();
//   packageCountdownTimer = setInterval(tick, 60 * 1000); // تحديث كل دقيقة
// }

// /* ========== Package UI refresh (with countdown) ========== */
...
//       dt.setHours(23,59,59,999); // نهاية اليوم
//       return dt;
//     }
...
//     const hint = document.getElementById('activateHint');
//     const btn = document.getElementById('activatePackageBtn');
//     if (!logged || !logged.id || !btn) return;
...
//     const pkgStatusRaw = place.raw['حالة الباقة'] || place.raw['packageStatus'] || '';
...
//     const pkgStatus = String(pkgStatusRaw || '').trim();
...
//     if (!pkgStatus || pkgStatus === '') {
//       // لا توجد حالة — لا يوجد اشتراك
//       clearPackageCountdown();
...
//       btn.textContent = 'الاشتراك مُفعّل';
//       let msg = 'حالة الباقة: نشطة';
//       if (startDate && endDate) {
//         const startTxt = startDate.toISOString().split('T')[0];
//         const endTxt = endDate.toISOString().split('T')[0];
//         if (remaining !== null && remaining < 0) remaining = 0;
//         msg += ` — البداية: ${startTxt} · النهاية: ${endTxt}${remaining !== null ? ` · المتبقي: ${remaining} يوم` : ''}`;
//       }
//       if (hint) { hint.textContent = msg; hint.classList.add('active'); }
//       // ابدأ العدّاد التنازلي إذا أقل من 30 يوم متبقية
//       if (endDate) {
//         const daysLeft = daysBetween(today, endDate);
//         if (daysLeft !== null && daysLeft <= 30) {
//           startPackageCountdown(endDate);
//         } else {
//           clearPackageCountdown();
//         }
//       } else {
...
//       btn.disabled = true;
//       btn.style.opacity = '0.8';
//       btn.textContent = 'قيد التحقق من الدفع';
...
//         const startTxt = startDate.toISOString().split('T')[0];
//         const endTxt = endDate.toISOString().split('T')[0];
//         msg += ` — البداية: ${startTxt} · النهاية: ${endTxt}`;
//       }
...
//     btn.disabled = false;
//     btn.style.opacity = '1';
//     btn.textContent = (pkgStatus.indexOf('منته') !== -1) ? 'تجديد الاشتراك' : 'تفعيل الاشتراك';
...
//     // ترك تحديد تفعيل/تعطيل الزر النهائي لدالة refreshPackageUIFromDashboard (حسب حالة الباقة)
//     // هنا فقط عرض تلميح بسيط لحين التحديث
//     if (!hasPackageSelection) {
//       if (hint) {
//         const status = (getLoggedPlace()?.raw?.['حالة الباقة'] || '').trim();
//         hint.textContent = 'حالة الباقة: ' + (status || 'لا يوجد اشتراك');
//         hint.classList.remove('active','pending','expired');
//       }
//       // ثم نحسم الحالة من الشيت
//       refreshPackageUIFromDashboard();
//       return;
//     }
...
//         if (used) {
//           btn.disabled = true; btn.style.opacity = '0.6';
//           if (hint) { hint.textContent = 'الباقة التجريبية تم تفعيلها مسبقًا.'; hint.classList.remove('active','pending','expired'); }
//         } else {
//           btn.disabled = false; btn.style.opacity = '1';
...
//   // حالة زر التفعيل الأولية: أخفِه إن لم يكن مسجلاً
//   applyActivateButtonVisibilityRules();
// });
...
//   const pkgSelect = document.querySelector('select[name="package"]');
//   if (pkgSelect) pkgSelect.addEventListener('change', () => {
//     // عند تغيير الاختيار، نحدّث حالة الزر والتلميحات فقط
//     updateActivateButtonState();
//     applyActivateButtonVisibilityRules();
//   });

//   // زر رفع الإيصال الديناميكي (يُنشأ عند قيد الدفع)
//   document.addEventListener('click', (e) => {
//     if (e.target && e.target.id === 'uploadReceiptBtn') {
//       e.preventDefault();
//       openReceiptUploadFlow();
//     }
//   });
// }
...
//       workingHours: formData.get('workingHours'),
//       delivery: formData.get('delivery'),
//       package: formData.get('package'), // لن نرسله في الحفظ
//       description: formData.get('description'),
//       password: formData.get('password'),
//       status: formData.get('status'),
//       image: uploadedImages[0] || null
...
//     setIf('delivery', placeData.delivery);
//     setIf('description', placeData.description);
//     // لا نرسل packageId هنا حتى لا نكسر منطق التفعيل
//     setIf('password', placeData.password);
//     setIf('logoUrl', imageUrl);
//     setIf('status', placeData.status);
...
//     // بعد أول حفظ ناجح، أظهر زر التفعيل إن كان مخفيًا
//     applyActivateButtonVisibilityRules();
//     await refreshPackageUIFromDashboard();
//     loadPlacesForAds();
...
//     if (!logged || !logged.id) {
//       if (hint) hint.textContent = 'احفظ بيانات المكان أولاً لإنشاء الحساب ثم اختر الباقة';
//       if (btn) { btn.textContent = 'تفعيل الاشتراك'; btn.disabled = true; }
//       return;
//     }
...
//     if (!pkgStatus) {
//       clearPackageCountdown();
//       if (hint) hint.textContent = 'اختر باقة ثم اضغط تفعيل الاشتراك';
//       // الزر يظهر فقط عند اختيار باقة (سيعالجها applyActivateButtonVisibilityRules)
//       showCards();
//       if (cardText) cardText.textContent = 'باقتك الحالية: لا يوجد اشتراك';
//       if (inlineText) inlineText.textContent = 'باقتك الحالية: لا يوجد اشتراك';
//       return;
...
//           msg += ` — البداية: ${sTxt} · النهاية: ${eTxt}${remaining !== null ? ` · المتبقي: ${remaining} يوم` : ''}`;
//         }
//         hint.textContent = msg; hint.classList.add('active');
//       }
//       showCards();
//       const pn = packageName || (pkgId ? `ID ${pkgId}` : 'غير معروفة');
//       const eTxt = endDate ? endDate.toISOString().split('T')[0] : '';
...
//       if (hint) {
//         let msg = 'حالة الباقة: منتهية';
//         if (startDate && endDate) msg += ` — البداية: ${startDate.toISOString().split('T')[0]} · النهاية: ${endDate.toISOString().split('T')[0]}`;
//         hint.textContent = msg; hint.classList.add('expired');
//       }
//       showCards();
//       const pn = packageName || (pkgId ? `ID ${pkgId}` : 'غير معروفة');
//       const eTxt = endDate ? endDate.toISOString().split('T')[0] : '';
...
//     console.warn('refreshPackageUIFromDashboard error', e);
//   } finally {
//     // بعد تحديث الواجهة، طبّق قواعد ظهور الزر
//     applyActivateButtonVisibilityRules();
//     updateActivateButtonState();
//   }
// }
...
//     const trialUsed = String(place.raw['حالة الباقة التجريبية']).toLowerCase() === 'true';
//     const pkgStatus = String(place.raw['حالة الباقة'] || '').trim();
//     // نمنع التجريبية فقط إذا كانت مستخدمة سابقاً وحالة الباقة الحالية "منتهية"
//     if (trialUsed && pkgStatus === 'منتهية') return true;
//     return false;
//   } catch (e) {
//     console.warn('checkIfTrialIsUsed error', e);
...
//     if (!loggedPlace || !loggedPlace.id) {
//       showError('يجب حفظ المكان أو تسجيل الدخول أولاً');
//       return;
//     }
//     if (!packageSelect || !packageSelect.value) {
//       showError('يجب اختيار باقة أولاً من القائمة');
...
// // ========== ACTIVATE BUTTON VISIBILITY RULES ==========
// function applyActivateButtonVisibilityRules() {
//   const btn = document.getElementById('activatePackageBtn');
//   const hint = document.getElementById('activateHint');
//   if (!btn) return;

//   const logged = getLoggedPlace();
//   const loggedIn = !!(logged && logged.id);
//   const pkgSelect = document.querySelector('select[name="package"]');
//   const hasSelection = !!(pkgSelect && pkgSelect.value && String(pkgSelect.value).trim() !== '');

//   const status = (logged && logged.raw && logged.raw['حالة الباقة']) ? String(logged.raw['حالة الباقة']).trim() : '';

//   // إظهار زر دفع الإيصال عند قيد الدفع
//   ensureReceiptButtonExists(status === 'قيد الدفع');
...
//   if (status === 'قيد الدفع') {
//     btn.style.display = 'none';
//     if (hint) hint.textContent = 'طلب الدفع قيد المراجعة. يمكنك رفع الإيصال.';
//     return;
//   }

//   // في الحالات الأخرى، نظهر الزر
//   btn.style.display = 'inline-block';

//   if (!hasSelection && !status) {
//     // لا يوجد اشتراك ولم يتم اختيار باقة
//     btn.disabled = true;
//     btn.style.opacity = '0.6';
//     btn.textContent = 'تفعيل الاشتراك';
//     if (hint) hint.textContent = 'اختر باقة أولاً';
//     return;
//   }
...
//     btn.style.opacity = '0.8';
//     btn.textContent = 'الاشتراك مُفعّل';
//     if (hint && (!hint.textContent || hint.textContent.trim() === '')) hint.textContent = 'الاشتراك نشط حالياً';
//     return;
//   }

//   if (status === 'منتهية') {
//     btn.disabled = !hasSelection ? false : false;
//     btn.style.opacity = '1';
//     btn.textContent = 'تجديد الاشتراك';
//     if (hint && (!hint.textContent || hint.textContent.trim() === '')) hint.textContent = 'اختر باقة ثم اضغط تجديد';
//     return;
//   }

//   // الحالة الافتراضية: لا يوجد اشتراك مع اختيار باقة
//   btn.disabled = !hasSelection;
//   btn.style.opacity = hasSelection ? '1' : '0.6';
//   btn.textContent = 'تفعيل الاشتراك';
//   if (hint && (!hint.textContent || hint.textContent.trim() === '')) hint.textContent = hasSelection ? '' : 'اختر باقة أولاً';
// }

// function ensureReceiptButtonExists(show) {
//   let btn = document.getElementById('uploadReceiptBtn');
//   const container = document.getElementById('savePlaceBtn')?.parentElement;
//   const hint = document.getElementById('activateHint');
//   if (!container) return;
...
//       btn.id = 'uploadReceiptBtn';
//       btn.className = 'btn btn-secondary';
//       btn.style.marginInlineStart = '6px';
//       btn.innerHTML = '<i class="fas fa-receipt"></i> رفع إيصال الدفع';
//       container.insertBefore(btn, hint || null);
//     }
//     btn.style.display = 'inline-block';
//   } else {
...
// async function openReceiptUploadFlow() {
//   try {
//     const logged = getLoggedPlace();
//     if (!logged || !logged.id) { showError('سجّل الدخول أولاً'); return; }
//     // نحاول الحصول على آخر طلب دفع إن توفر
//     let paymentId = '';
//     try {
//       const r = await apiPost({ action: 'getLastPaymentRequest', placeId: logged.id });
//       if (r.ok) {
//         const d = r.data && r.data.data ? r.data.data : r.data;
//         paymentId = d && (d.paymentId || d.id) ? (d.paymentId || d.id) : '';
//       }
//     } catch(e){}

//     showPaymentModal({ paymentId, amount: '', currency: 'SAR', placeId: logged.id });
//   } catch (e) {
//     console.error('openReceiptUploadFlow error', e);
//     showError('تعذر فتح نافذة رفع الإيصال');
//   }
// }

// // ========== ACTIVATE BUTTON STATE (ENABLED/DISABLED + HINTS) ==========
// function updateActivateButtonState() {
//   try {
//     const btn = document.getElementById('activatePackageBtn');
//     const hint = document.getElementById('activateHint');
...
//     // لو الزر مخفي بسبب قواعد الظهور، لا داعي للمتابعة
//     if (btn.style.display === 'none') return;

//     const hasLogin = !!(logged && logged.id);
//     const hasPackageSelection = !!(select && select.value && String(select.value).trim() !== '');
//     if (!hasLogin) {
//       if (hint) { hint.textContent = 'احفظ أولاً لإنشاء الحساب ثم اختر الباقة'; hint.classList.remove('active','pending','expired'); }
//       btn.disabled = true; btn.style.opacity = '0.6';
//       return;
//     }
...
//     if (status === 'قيد الدفع') {
//       btn.disabled = true; btn.style.opacity = '0.6';
//       if (hint) { hint.textContent = 'قيد الدفع — يمكنك رفع الإيصال'; hint.classList.remove('active','pending','expired'); }
//       return;
//     }

//     if (!hasPackageSelection) {
//       if (status) {
//         // هناك حالة موجودة بالفعل
//         if (status === 'نشطة') { btn.disabled = true; btn.style.opacity = '0.8'; btn.textContent = 'الاشتراك مُفعّل'; }
//         else if (status === 'منتهية') { btn.disabled = false; btn.style.opacity = '1'; btn.textContent = 'تجديد الاشتراك'; }
//       } else {
//         btn.disabled = true; btn.style.opacity = '0.6'; btn.textContent = 'تفعيل الاشتراك';
//       }
//       if (hint && (!hint.textContent || hint.textContent.trim() === '')) hint.textContent = 'اختر باقة أولاً';
//       return;
//     }

//     // تحقق التجريبية إذا الباقة مجانية وحالة منتهية
//     const selectedOpt = select.options[select.selectedIndex];
//     const selPrice = Number(selectedOpt?.dataset?.price || 0);
//     if (selPrice === 0) {
//       checkIfTrialIsUsed(logged.id).then(used => {
//         if (used) {
//           btn.disabled = true; btn.style.opacity = '0.6';
//           btn.textContent = (status === 'منتهية') ? 'التجريبية غير متاحة' : 'تفعيل الاشتراك';
//           if (hint) { hint.textContent = 'لا يمكن تفعيل الباقة التجريبية مرة أخرى بعد انتهائها.'; hint.classList.remove('active','pending','expired'); }
//         } else {
//           btn.disabled = false; btn.style.opacity = '1';
//           btn.textContent = (status === 'منتهية') ? 'تجديد الاشتراك' : 'تفعيل الاشتراك';
//           if (hint && status === '') hint.textContent = '';
//         }
//       }).catch(()=>{
//         btn.disabled = false; btn.style.opacity = '1';
//         btn.textContent = (status === 'منتهية') ? 'تجديد الاشتراك' : 'تفعيل الاشتراك';
...
//       // لو لدينا paymentId، نربط الإيصال بالطلب
//       if (paymentId) {
//         const updatePayload = {
//           action: 'updatePaymentRequest',
//           paymentId: paymentId,
...
//     // تعبئة شبكة الباقات فقط (لا يوجد حقل باقة في النموذج)
//     const pkgGrid = document.getElementById('packagesGrid');
//     if (pkgGrid) {
//       pkgGrid.innerHTML = '';
//       (data.packages || []).forEach(p => {
...
//           const priceNum = Number(p.price || (p.raw && (p.raw['سعر الباقة'] || p.raw['السعر'])) || 0) || 0;
//           if (priceNum === 0) {
//             const block = await checkIfTrialIsUsed(logged.id);
//             if (block) { showError('الباقة التجريبية تم تفعيلها مسبقًا بعد انتهاء اشتراك سابق. لا يمكن تفعيلها مرة أخرى.'); return; }
//           }
//           await choosePackageAPI(p.id);
//         };
//         div.appendChild(h); div.appendChild(d); if (desc.textContent) div.appendChild(desc); div.appendChild(btn);
//         pkgGrid.appendChild(div);
//       });
...
//     setIf('password', placeData.password);
//     setIf('logoUrl', imageUrl);
//     // لا نرسل status ولا packageId من النموذج
...
//     // لا نسمح بالانتقال للباقات إلا بعد الحفظ/تسجيل الدخول
//     const logged = getLoggedPlace();
//     if (!logged || !logged.id) {
...
// /* Ads submit/list (unchanged core) */
// async function handleAdSubmit(ev) {
//   ev.preventDefault();
//   showLoading(true);
//   try {
...
//   // خزّن الاسم داخل الكائن لضمان ظهوره لاحقاً
...
//     // طبّق اسم المكان في حال كان داخل raw فقط
...
//   });
// }
// function showSuccess(message) { const el = document.getElementById('successAlert'); if (!el) return; el.textContent = message; el.className = 'alert alert-success'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',4000); }
// function showError(message) { const el = document.getElementById('errorAlert'); if (!el) return; el.textContent = message; el.className = 'alert alert-error'; el.style.display = 'block'; setTimeout(()=>el.style.display='none',5000); }
// function showLoading(show) { const el = document.getElementById('loading'); if (!el) return; el.style.display = show ? 'block' : 'none'; }
...
//     const logoUrl = raw['رابط صورة شعار المكان'] || raw['صورة شعار أو صورة المكان'] || '';
//     if (logoUrl) {
//       const preview = document.getElementById('placeImagePreview'); if (preview) { preview.innerHTML = ''; const img = document.createElement('img'); img.src = logoUrl; preview.appendChild(img); }
//     }
...
//     const countdown = document.getElementById('packageInfoCountdown');
//     if (!card || !text || !countdown) return;
//     card.style.display = 'none'; text.textContent = ''; countdown.textContent = ''; clearInterval(countdown._timer);
...
// /* Files validation (fix for "validateFiles is not defined") */
// function validateFiles() {
//   const maxImageSize = 10 * 1024 * 1024;  // 10MB
//   const maxVideoSize = 50 * 1024 * 1024;  // 50MB
//   const allowedImageTypes = ['image/jpeg','image/png','image/gif','image/webp'];
//   const allowedVideoTypes = ['video/mp4','video/avi','video/mov','video/quicktime','video/x-msvideo','video/ogg','video/webm'];

//   if (Array.isArray(uploadedImages)) {
//     for (let image of uploadedImages) {
//       if (!image) continue;
//       if (image.size > maxImageSize) { showError('حجم الصورة أكبر من 10MB'); return false; }
//       if (image.type && !allowedImageTypes.includes(image.type)) { showError('نوع الصورة غير مدعوم'); return false; }
//     }
//   }

//   if (Array.isArray(uploadedVideos) && uploadedVideos.length > 0) {
//     const v = uploadedVideos[0];
//     if (v) {
//       if (v.size > maxVideoSize) { showError('حجم الفيديو أكبر من 50MB'); return false; }
//       if (v.type && !allowedVideoTypes.includes(v.type)) { showError('نوع الفيديو غير مدعوم'); return false; }
//     }
//   }
//   return true;
// }
...
//     if (!validateFiles()) { showLoading(false); if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = oldHtml; } return; }
...
// /**
//  * تتحقق من صحة الصور (وحالياً الفيديو) قبل الإرسال:
//  * - حجم الصورة ≤ 10MB
//  * - أنواع الصور المدعومة: jpeg, png, gif, webp
//  * - حجم الفيديو ≤ 50MB
//  * - أنواع الفيديو المدعومة: mp4, avi, mov, quicktime
//  */
// function validateFiles() {
//   const maxImageSize = 10 * 1024 * 1024;   // 10MB
//   const maxVideoSize = 50 * 1024 * 1024;   // 50MB
...
//     if (vid.size > maxVideoSize) {
//       showError('حجم الفيديو أكبر من 50MB');
//       return false;
//     }
//     if (!allowedVideoTypes.includes(vid.type)) {
...
async function choosePackageAPI(packageId, options = {}) {
  ...
  try {
    if (price === 0) {
      ...
      const resp = await apiPost({ action: 'activateFreePackage', placeId: logged.id, packageId });
